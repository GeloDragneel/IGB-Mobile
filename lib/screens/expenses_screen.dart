...
class _ExpensesScreenState extends State<ExpensesScreen>
  ...
  Future<List<ExpenseRecord>> _futureRecords = Future.value([]);
  List<ExpenseRecord> _allRecords = [];
  List<ExpenseRecord> _filteredRecords = [];
  int _currentPage = 1;
  ...
  Future<List<ExpenseRecord>> _fetchRecords() async {
    final auth = Provider.of<AuthProvider>(context, listen: false);
    final res = await http.get(
      Uri.parse(
        'https://igb-fems.com/LIVE/mobile_php/expenses.php'
  ...
  List<ExpenseRecord> get _pageRecords {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: _buildAppBar(),
      drawer: AppDrawer(selectedIndex: 5),
      body: FutureBuilder<List<ExpenseRecord>>(
      ...
      child: Container(
        ...
        child: Material(
          ...
          child: InkWell(
            borderRadius: BorderRadius.circular(14),
            onTap: () {},
            splashColor: _T.amber.withOpacity(0.05),
            child: Padding(
...
class _ViewScreenState extends State<ViewScreen> {
  ...
  Future<void> _fetchInvoice() async {
    try {
      final res = await http.get(
        Uri.parse(
          'https://igb-fems.com/LIVE/mobile_php/expenses_info.php'
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ? const Center(child: CircularProgressIndicator(color: _T.amber))
          : error.isNotEmpty
          ? Center(
              child: Text(
                'Error: $error',
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [_T.amber.withOpacity(0.18), _T.card],
    ...
    return Row(
      ...
      children: [
        Container(
          ...
          child: Icon(icon, color: _T.amber, size: 14),
        ),
        const SizedBox(width: 10),
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
