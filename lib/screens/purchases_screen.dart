...
import 'purchase_scan_dialog.dart';
...
class _PurchasesScreenState extends State<PurchasesScreen> {
  ...
  Future<List<PurchaseRecord>> _futurePurchaseRecords = Future.value([]);
  int _currentPage = 1;
  final int _rowsPerPage = 10;
  List<PurchaseRecord> _allRecords = [];
  List<PurchaseRecord> _filteredRecords = []; // 🔍 filtered list
  String _searchQuery = ""; // 🔍 search text

  @override
  void initState() {
    super.initState();
    _futurePurchaseRecords = fetchPurchaseRecords();
  }
  ...
  Future<List<PurchaseRecord>> fetchPurchaseRecords() async {
    ...
    final response = await http.get(
      Uri.parse(
        'https://igb-fems.com/LIVE/mobile_php/purchases.php?userId=${auth.userId}&from=${auth.fromDate}&to=${auth.toDate}',
      ),
    );
    if (response.statusCode == 200) {
      ...
      if (data['result'] == 'Success') {
        ...
        final records = invoices
            .map((inv) => PurchaseRecord.fromJson(inv))
            .toList();
        _allRecords = records;
        _filteredRecords = records; // initialize filtered list
        return records;
      } else {
        throw Exception('Failed to load purchases records: ${data['result']}');
      }
    } else {
      throw Exception(
        'Failed to load purchases records: ${response.statusCode}',
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: AppDrawer(selectedIndex: 4),
              ...
              child: FutureBuilder<List<PurchaseRecord>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    return const Center(
                      child: Text('No purchase records available'),
                    );
                  } else {
...
class _ViewScreenState extends State<ViewScreen> {
  ...
  Future<void> fetchInvoice() async {
    final url =
        'https://igb-fems.com/LIVE/mobile_php/purchases_info.php?InvoiceNo=${widget.invoiceNo}&UserID=${widget.userId}';
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Purchases / Invoice'),
        backgroundColor: const Color(0xFF121826),
        foregroundColor: Colors.white,
      ),
      backgroundColor: const Color(0xFF121826),
